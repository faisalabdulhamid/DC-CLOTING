	...
	<div>
		...
		<div class="panel panel-default">
	        ...
	        <div class="panel-body"> 
				<form class="form-horizontal" v-on:submit.prevent="handleSave" enctype="multipart/form-data">
					...
					<div class="form-group">
						<label for="nama" class="control-label col-md-3">Gambar</label>
						<div class="col-md-9">
							<input type="file" id="gambar" multiple="">
						</div>
					</div>
	...
	export default{
		...
		methods: {
			handleSave (){
				...
				if (typeof this.id !== 'undefined') {
					this.$http.post(`/dc/promosi/${this.id}`, formData, {
						headers: {
					...
					this.$http.post(`/dc/promosi`, formData, {
						headers: {
							Authorization: `Bearer ${this.login_admin.access_token}`
						}
					})
		...
		watch: {
			'endTime.time': function () {
				this.form.akhir_promosi = this.endTime.time
			},
			'startTime.time': function () {
				this.form.mulai_promosi = this.startTime.time
			},
		},
		created () {
			if (typeof this.id !== 'undefined') {
